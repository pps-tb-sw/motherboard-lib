...
namespace PPSTimingMB
{
  ...
  std::ostream& operator<<(std::ostream& out, const TDCSetupRegister& sp);
  inline unsigned short GetNumTDCSetupRegisters() { return rNumSetupRegisters; }
  /**
  ...
  class TDCSetup : public TDCRegister
  {
    ...
    public:
      ...
      inline TDCSetup(const TDCSetup& c) : TDCRegister(TDC_SETUP_BITS_NUM, c) {
        SetConstantValues();
      }
      inline TDCSetup(const std::vector<uint8_t>& v, bool reverse=false) : TDCRegister(TDC_SETUP_BITS_NUM, v, reverse) {}
